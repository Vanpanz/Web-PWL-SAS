<!DOCTYPE html>
<html lang="id">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>LamarKuy! - Portal Kerja</title>
  <link rel="stylesheet" href="Landing page/style.css">
</head>
<body>
  
  <?php include "./header/header.php";?> 


  
 <!-- Hero Banner -->
  <section class="hero">
    
    <h1>Bingung Mulai Dari Mana? Cari Di Sini!</h1>
    <div class="search-box">
      <input type="text" placeholder="Perusahaan">
      <input type="text" placeholder="Bidang">
      <input type="text" placeholder="Lokasi">
      <button class="btn-primary">Cari</button>
    </div>
  </section>

  <!-- lowongan -->
<section class="lowongan">
  <h2>Yang Mungkin Cocok Denganmu</h2>
  <div class="job-list-rows">

    <a href="formlamaran.html" class="job-row">
      <div class="job-info">
        <h3>UI/UX Designer</h3>
        <p>PT. ABC, Kalimantan Barat</p>
      </div>
      <div class="job-location">Pontianak Selatan</div>
      <div class="job-action">
        <button class="btn-primary">Lamar</button>
      </div>
    </a>

    <a href="formlamaran.html" class="job-row">
      <div class="job-info">
        <h3>Front End Developer</h3>
        <p>Kantor CDE, Jawa Tengah</p>
      </div>
      <div class="job-location">Magelang</div>
      <div class="job-action">
        <button class="btn-primary">Lamar</button>
      </div>
    </a>

    <a href="formlamaran.html" class="job-row">
      <div class="job-info">
        <h3>Back End Developer</h3>
        <p>Kantor CDE, Jawa Tengah</p>
      </div>
      <div class="job-location">Semarang</div>
      <div class="job-action">
        <button class="btn-primary">Lamar</button>
      </div>
    </a>

  </div>
</section>

  <!-- Kategori Populer -->

  <div class="carousel-container" >
   <div class="carousel-track" id="carousel-track">
            <div class="carousel-item">
            <div class="carousel-card">
              <div class="img-wrapper">
                 <img src="../IMG FOLDER/deliote.png" alt="Deloitte">
              </div>
                <p>“Tips & Trik Berkarir Sebagai Akuntan Sukses”</p>
                <span>Periode : 17 Agustus - 19 Agustus 2025</span>
            </div>
            </div>
            <div class="carousel-item">
            <div class="carousel-card">
              <div class="img-wrapper">
                <img src="../IMG FOLDER/image-removebg-preview (1).png" alt="Coda Payments">
              </div>
                <p>“Menjadi Pengembang Layanan Yang Cerdas”</p>
                <span>Periode : 18 Agustus - 21 Agustus 2025</span>
              </div>
            </div>

            <div class="carousel-item">
            <div class="carousel-card">
              <div class="img-wrapper">
                <img src="../IMG FOLDER/Traveloka.png" alt="Traveloka">
              </div>
                <p>“Satu Langkah Kecil Membawa Perubahan Besar”</p>
                <span>Periode : 16 Agustus - 20 Agustus 2025</span>
            </div>
            </div>

            <div class="carousel-item">
            <div class="carousel-card">
              <div class="img-wrapper">
                <img src="../IMG FOLDER/Coway (1).jpg" alt="Coway">
              </div>
                <p>"Jernihkan Pikiran Layaknya Air Bersih</p>
                <span>Periode : 13 Agustus - 14 Agustus 2025</span>
              </div>
            </div>

           <div class="carousel-item">
            <div class="carousel-card">
              <div class="img-wrapper">
                <img src="IMG FOLDER/Manulife.jpg" alt="Manulife">
              </div>
                 <p>"Bangun Kepercayaan Diri Dari Asuransi"</p>
                <span>Periode : 13 Agustus - 15 Agustus 2025</span>
                </div>
            </div>

           <div class="carousel-item">
            <div class="carousel-card">
              <div class="img-wrapper">
              <img src="../IMG FOLDER/jiva.jpg" alt="jiva">
              </div>
              <p>"Mengembangkan Teknologi Pertanian Untuk Masa Depan</p>
              <span>Periode : 16 Agustus - 20 Agustus 2025</span>
            </div>
            </div>

            <div class="carousel-item">
            <div class="carousel-card">
              <div class="img-wrapper">
              <img src="../IMG FOLDER/ASRI.jpg" alt="ASRI">
              </div>
              <p>"Mengelola Property Kepunyaan Dengan Bijak"</p>
              <span>Periode : 11 Agustus - 15 Agustus 2025 </span>
              </div>
            </div>

           <div class="carousel-item">
            <div class="carousel-card">
              <div class="img-wrapper">
              <img src="../IMG FOLDER/tiket.com.jpg" alt="tiket">
              </div>
              <p>"Terbang Ke Atas Melampaui Segalanya"</p>
              <span>Periode : 12 Agustus - 16 Agustus 2025</span>
              </div>
            </div>

            <div class="carousel-item">
            <div class="carousel-card">
              <div class="img-wrapper">
              <img src="../IMG FOLDER/Gojek_225x225-removebg-preview.png" alt="gojek">
              </div>
              <p>"Telusuri Bakatmu, Kelilinglah Dunia!"</p>
              <span>Periode : 16 Agustus - 17 Agustus 2025</span>
            </div>
            </div>
        </div>
    </div>
  </div>

    




  <script src="script.js"></script>

  <section class="Pojok Tips Karir">
    <div class="text2">
      <h2>Pojok Tips Karir</h2>
    </div>
   

  <div class="tips-container">
    <div class="tip-box">
      <img src="../IMG FOLDER/icon like.png" alt="">
      <p>Selalu tonjolkan kelebihanmu dan prestasimu pada CV</p>
    </div>
    <div class="tip-box">
      <img src="../IMG FOLDER/icon dislike.png" alt="">
      <p>Hindarilah menulis pengalaman yang tidak relevan dengan posisi yang dilamar</p>
    </div>
    <div class="tip-box">
      <img src="../IMG FOLDER/icon send.png" alt="">
      <p>Jangan menunda-nunda dalam mencari pekerjaan, kirim CV mu secepatnya!</p>
    </div>
    <div class="tip-box">
      <img src="../IMG FOLDER/icon love.png" alt="">
      <p>Carilah pekerjaan yang sesuai dengan minat dan bakatmu</p>
    </div>
    <div class="tip-box">
      <img src="../IMG FOLDER/icon kerja.png" alt="">
      <p>Bangunlah profil diri yang professional dan kuat dalam CV mu</p>
    </div>
    <div class="tip-box">
      <img src="../IMG FOLDER/icon pesan.png" alt="">
      <p>Latihlah kemampuan berkomunikasimu, jago komunikasi jago kerja</p>
    </div>
  </div>
  </section>

  <section class="Ini Cerita Kita">
    <div class="textini"
    <h1>Ini Cerita Kita</h1>
    </div>

  <div class="card-container">
    <div class="card">
      <div class="avatar"></div>
      <div class="text">
        <div class="name">Selvin Agustino</div>
        <div class="lines"></div>
        <div class="lines"></div>
      </div>
    </div>

    <div class="card">
      <div class="avatar"></div>
      <div class="text">
        <div class="name">Ryu Felix Dariell</div>
        <div class="lines"></div>
        <div class="lines"></div>
      </div>
    </div>

    <div class="card">
      <div class="avatar"></div>
      <div class="text">
        <div class="name">Evan Fernando Fransiskus</div>
        <div class="lines"></div>
        <div class="lines"></div>
      </div>
    </div>

    <div class="card">
      <div class="avatar"></div>
      <div class="text">
        <div class="name">Felix Fransiskus</div>
        <div class="lines"></div>
        <div class="lines"></div>
      </div>
    </div>

    <div class="card">
      <div class="avatar"></div>
      <div class="text">
        <div class="name">Agus Tino</div>
        <div class="lines"></div>
        <div class="lines"></div>
      </div>
    </div>

    <div class="card">
      <div class="avatar"></div>
      <div class="text">
        <div class="name">Riyu Nando</div>
        <div class="lines"></div>
        <div class="lines"></div>
      </div>
    </div>
  </div>

  <div class="input-section">
    <a href="tulis-cerita.html" class="link-input">Tulis Cerita Sukses Versi Kamu Di Sini...</a>
    <button>Lihat Selengkapnya</button>
  </div>
  </section>
  </section>

  <footer>
  <div class="footer-container">
    <div class="footer-left">
      <div class="footer-logo-row">
        <img src="../IMG FOLDER/logo lamar.png" alt="LamarKuy Logo">
        <div class="footer-logo-text">
          <span class="footer-brand">LamarKuy!</span>
          <span class="footer-slogan">Achieve Higher, Growth Better.</span>
        </div>
      </div>
      <div class="footer-community">
        <span>Our Community</span>
        <div class="social-icons">
          <a href="#"><img src="../IMG FOLDER/twite.png" alt="X"></a>
          <a href="#"><img src="../IMG FOLDER/discord.png" alt="Discord"></a>
          <a href="#"><img src="../IMG FOLDER/instagram.png" alt="Instagram"></a>
          <a href="#"><img src="../IMG FOLDER/youtube.png" alt="YouTube"></a>
        </div>
      </div>
    </div>
    <div class="footer-links">
      <div>
        <h4>Tools</h4>
        <ul>
          <li><a href="#">Membuat Resume</a></li>
          <li><a href="#">Membuat CV</a></li>
          <li><a href="#">Membuat Surat Lamaran</a></li>
          <li><a href="#">Penyedia Pekerjaan</a></li>
          <li><a href="#">Pelamaran</a></li>
        </ul>
      </div>
      <div>
        <h4>Resources</h4>
        <ul>
          <li><a href="#">Template Resume</a></li>
          <li><a href="#">Contoh Resume</a></li>
          <li><a href="#">Template CV</a></li>
          <li><a href="#">Contoh CV</a></li>
          <li><a href="#">Contoh Surat Lamaran</a></li>
        </ul>
      </div>
      <div>
        <h4>Explore</h4>
        <ul>
          <li><a href="#">Tips Karir</a></li>
          <li><a href="#">Contoh CV Bagus</a></li>
          <li><a href="#">Surat Lamaran Kerja</a></li>
          <li><a href="#">Karir Yang Sedang Tren</a></li>
          <li><a href="#">Apa Syarat Resume Bagus</a></li>
        </ul>
      </div>
    </div>
  </div>
  <div class="footer-bottom">
    <div class="footer-bottom-copyright">
    <p>© 2025 LamarKuy!</p>
    </div>
    <div class="footer-bottom-links">
      <a href="#">Cookie Policy</a>
      <a href="#">Persyaratan dan Ketentuan</a>
      <a href="#">Keamanan dan Privasi</a>
    </div>
  </div>
</footer>
</body>
</html>